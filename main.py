...
base_configs = [CONFIG_1] # Enter confits to queue experiments for
    ...
    for base_config in base_configs:
        ...
        for seed in seeds:
            ...
            for val_fold in cv_folds:
                ...
                config["val_fold"] = val_fold
                config["seed"] = seed
                config["experiment_name"] = f"{name_identifier}_seed{seed}_cv_valfold{val_fold}"
                run_experiment(config)
            ...
            config["test_fold"] = test_fold
            config["seed"] = seed
            config["experiment_name"] = f"{name_identifier}_seed{seed}_test_fold{test_fold}"
            run_experiment(config)
